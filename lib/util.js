...
module.exports = {
  ...
  joinRelative: function (url, options) {
    // Remove suffix
    if (url.indexOf('?') !== -1) {
    ...
    if (url.startsWith('/')) {
      return path.join(options.root, url.slice(1))
    } else if (isFolder) {
      return path.join(options.path, url)
    } else if (url.startsWith('..') && !url.endsWith('md')) {
      return path.join(options.path, url.replace(/^\.\.\//, '../../'))
    } else if (url.startsWith('.') && !url.endsWith('md')) {
      return path.join(options.path, url.replace(/^\.\//, '../'))
    } else {
      console.log(path.join(options.path.split('/').slice(0, -2).join('/'), url))
      return path.join(options.path.split('/').slice(0, -2).join('/'), url)
    }
  },
