...
function compiler(options) {
  ...
  // Links: { location: string, children: content }
  const _links = new Array()

  // Index of current link
  let linkIndex = 0

  // Check if footnotes (references) are used in original markdown document
  let hasFootnotes = false

  // Indicates whether the current #details block is unwrapped
  let unwrap = false
...
`
    ...
    if (linkIndex > 0) {
      ...
      article += '#links-grid('
      for (const [i, link] of _links.entries()) {
        article += 'links-cell[\\[{0}\\] #link("{1}")[{2}]], qrcode("{1}"), '.format(
          i + 1,
  ...
  function parse(node) {
    const makeLink = function (url) {
      ...
      if (util.isInternalLink(url)) {
        ...
        return (location !== '' && raw !== '') ? ''.format(location, children) : ''
      } else {
        const location = url.replace(/\\/g, '\\\\')
        ++linkIndex
        _links.push({ location: location, children: children })
        ...
        // ++footnoteRefId[index]
        // if (location === raw) {
        return '#link("{0}")[{1}]#super[\\[{2}\\]]'.format(location, children, linkIndex)
        // } else {
    ...
    switch (node.type) {
      ...
      case 'html': {
        if (node.value == '<kbd>') {
          ...
          // HTML 标签（不含标签里的内容，如 <kbd>A</kbd> 会分别产生一个 html('<kbd>'), text('A'), html('</kbd>')，这里直接忽略掉就行
          return ''
        }
      }
      case 'link': {
