...
`.format(node.align.length,
      ...
      case 'details': { // Pymdown details 语法块
        const defaultTitle =
            (node.value === 'abstract') ? 'Abstract'
          : (node.value === 'info')     ? 'Info'
          : (node.value === 'tip')      ? 'Tip'
          : (node.value === 'success')  ? 'Success'
          : (node.value === 'question') ? 'Question'
          : (node.value === 'warning')  ? 'Warning'
          : (node.value === 'failure')  ? 'Failure'
          : (node.value === 'danger')   ? 'Danger'
          : (node.value === 'bug')      ? 'Bug'
          : (node.value === 'example')  ? 'Example'
        ...
        return '#details(unwrap: {0}, type: "{1}")[{2}][{3}]\n'.format(
          unwrap,
          node.value,
          title,
          util.all(node, parse).join(''))
