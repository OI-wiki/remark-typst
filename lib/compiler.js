...
import { all, isInternalLink, toPrefix, joinRelative, isUrl, trailingLineFeed,
  forceLinebreak, escapeAsString, checkCodeLang, capitalize, unquote } from './util.js'
...
function toTypst(tree, options) {
  ...
  // Item: { location: string, children: content }
  ...
  function parse(node) {
    ...
    switch (node.type) {
      ...
      case 'code': {
        const ret = '#codeblock(lang: "{1}", unwrapped: {2}, "{0}")\n'.format(
          escapeAsString(node.value) || '',
          checkCodeLang(node.lang),
          unwrap
        )
        unwrap = false
      ...
      case 'list': {
        ...
        const func = node.ordered
          ...
          : '#list'
        const ret = func + all(node, parse).map(s => '[{0}]'.format(s)).join('') + '\n'
      ...
      case 'table': {
        ...
        // TODO: repeat-header
        return TABLEX_TEMPLATE.format(node.align.length,
      ...
      case 'inlineMath': { // 行内公式
        ...
        if (texmath.status === 1) {
          ...
          const svg = renderSvg({}, {}).render(node.value, {})
          return '#inlinemath(svg: "{0}")'.format(svg.replace(/\"/g, '\\\"'))
        }
      ...
      case 'math': { // 行间公式
        ...
        if (texmath.status === 1) {
          ...
          const svg = renderSvg({}, {}).render(node.value, {})
          return '#dispmath(svg: "{0}")'.format(svg.replace(/\"/g, '\\\"'))
        }
