...
function toTypst(tree, options) {
  ...
  function parse(node) {
    ...
    switch (node.type) {
      ...
      case 'code': {
        const ret = '#codeblock("{0}", lang: "{1}", unwrapped: {2})\n'.format(
      ...
      case 'inlineMath': { // 行内公式
        ...
        const converted = texmath.stdout.toString('utf8').trimEnd()
        ...
        if (texmath.status === 1) {
          ...
          return '#svg-math("{0}")'.format(svg.replace(/\"/g, '\\\"'))
        }
      ...
      case 'math': { // 行间公式
        ...
        if (texmath.status === 1) {
          ...
          const svg = renderSvg({}, {}).render(node.value, {})
          return '#svg-math("{0}", display: true)'.format(svg.replace(/\"/g, '\\\"'))
        }

        const tags = node.value.match(/\\tag{(?<tagNumber>[0-9]+)}/)

        return tags
          ? `#math.equation(block: true, numbering: ignored => "(${tags.groups.tagNumber})", $ ${converted} $)`
          : `$ ${converted} $`
      }
