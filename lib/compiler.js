...
// const mathjax = require('../remark-mathjax/index')
...
function compiler(options) {
  ...
  // Unified containers
  // K-V: identifier - children nodes
  const _footnotes = new Map()
...
#import "@preview/tablex:0.0.4": tablex, hlinex
#import "../oi-wiki.typ": *
`
  ...
  function parseDefinition(tree) {
    visit(tree, 'footnoteDefinition', node => {
      _footnotes.set(node.identifier, node)
    })
    ...
    visit(tree, 'definition', node => {
      ...
      // }
      _definitions.set(node.identifier, node) 
    })
  ...
  function parse(node) {
    const makeLink = function (url) {
      ...
      // TODO: redirectable internal links (labels)
      // TODO: this is actually not fully implemented;
      // should we make every internal link redirectable?
      if (util.isInternalLink(url)) {
        ...
        // ++footnoteRefId[index]
        // if (location === raw) {
        return '#link("{0}")[{1}]'.format(location, children) + '#footnote[{0}]'.format(escape(location))
        // } else {
    ...
    const makeImage = function (loc) {
      // TODO: hugh, why should we copy all files into here,
    ...
    switch (node.type) {
      ...
      case 'text': {
        if (options.forceEscape) {
          return escape(util.forceLinebreak(node.value.trim()))
        } else {
          return escape(node.value.trim())
        }
      }
      case 'emphasis': {
        return '#emph[{0}]'.format(util.all(node, parse).join('').replace(/([ -~]+)/g, " $1 "))
      }
      case 'strong': {
        return '#strong[{0}]'.format(util.all(node, parse).join('').replace(/([ -~]+)/g, " $1 "))
      }
      case 'inlineCode': {
        return ' #raw("{0}", block: false) '.format(node.value.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/[\n]/g, '\\n'))
      }
      case 'code': {
        const content = '#codeblock(code: "{0}", lang: "{1}", unwrapped: {2})\n'.format(
          node.value.replace(/\\/g, '\\\\').replace(/"/g, '\\\"').replace(/\n/g, '\\n'),
          util.checkLanguage(node.lang),
          unwrap
        )
      ...
      case 'linkReference': {
        if (_definitions.get(node.identifier)) {
          return makeLink(_definitions.get(node.identifier).url)
        }
        return ''
      }
      case 'image': {
      ...
      case 'imageReference': {
        if (_definitions.get(node.identifier)) {
          return makeImage(_definitions.get(node.identifier).url)
        }
        return ''
      }
      case 'table': {
        ...
        // FIXME: repeat-header behaves oddly
        return `#tablex(
columns: ({0}),
...
hlinex(),
      ...
      case 'footnoteReference': {
        ...
        // ++footnoteRefId[index]
        return '#footnote[{0}]'.format(util.nonParagraphBegin(util.all(_footnotes.get(node.identifier), parse).join('')))
      }
      case 'definition': {
        return '' // 已经预处理掉了
      }
      ...
      case 'inlineMath': { // 行内公式
        const texmath = child_process.spawnSync(
          'texmath-string',
          ['--inline', '-f', 'tex', '-t', 'typst', node.value]
        )

        if (texmath.error) {
          // console.log(texmath.error)
          return ' $"MATH_NOT_SUPPORTED: {0}"$ '.format(
            texmath.stderr.toString('utf-8').replace(/\\/g, '\\\\').replace(/"/g, '\\\"').trimEnd())
        }

        return ` $${texmath.stdout.toString('utf-8').trimEnd()}$ `

        // TODO: svg approach?
        // return '#inlinemath(svg: "' + node.value.replace(/"/g, '\\\"') + '")'
      ...
      case 'math': { // 行间公式
        ...
        // TODO: svg approach?
        // return '#dispmath(svg: "' + node.value.replace(/"/g, '\\\"') + '")'
      ...
      case 'details': { // Pymdown details 语法块
        ...
        return '#details(unwrap: {0}, color: {1})[{2}][{3}]\n'.format(
          unwrap,
          color, 
          title, 
