...
function compiler(options) {
  ...
  // Indicates if the current details block is unwrapped
  let unwrap = false
    ...
    switch (node.type) {
      ...
      case 'linkReference': {
        if (definitions.get(node.identifier)) {
          return makeLink(definitions.get(node.identifier).url)
        }
        return ''
      }
      case 'image': {
      ...
      case 'imageReference': {
        if (definitions.get(node.identifier)) {
          return makeImage(definitions.get(node.identifier).url)
        }
        return ''
      }
      case 'table': {
...
hlinex(),
      ...
      case 'footnoteReference': {
        ...
        // ++footnoteRefId[index]
        return '#footnote[{0}]'.format(util.nonParagraphBegin(util.all(footnotes.get(node.identifier), parse).join('')))
      }
      case 'definition': {
        return '' // 已经预处理掉了
      }
